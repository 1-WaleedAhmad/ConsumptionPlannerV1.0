...
{
    public partial class Form11_WeeklySchedule : Form
    {
        ...
        public Form11_WeeklySchedule(int consumerID)
        {
            InitializeComponent();
            this.consumerID = consumerID;

            // Set the Dock property to make the DataGridView take the full width
            dataGridViewWeeklySchedule.Dock = DockStyle.Fill;

            // Load the weekly schedule when the form is loaded
            this.Load += new System.EventHandler(this.Form11_WeeklySchedule_Load);
        }

        private void Form11_WeeklySchedule_Load(object sender, EventArgs e)
        {
            LoadWeeklySchedule();

            // Adjust column widths after loading the data
            foreach (DataGridViewColumn column in dataGridViewWeeklySchedule.Columns)
            {
        ...
        private void LoadWeeklySchedule()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                // Fetch Appliances (Daily and Weekly)
                string queryAppliances = "SELECT * FROM Appliances WHERE ConsumerID = @ConsumerID AND Category IN ('Daily', 'Weekly')";
                using (SqlCommand cmd = new SqlCommand(queryAppliances, conn))
                {
                    ...
                    using (SqlDataAdapter adapter = new SqlDataAdapter(cmd))
                    {
                        ...
                        dataGridViewWeeklySchedule.DataSource = scheduleTable;
                    }
                }
            }
        }
