...
{
    public partial class Form10_DailySchedule : Form
    {
        ...
        public Form10_DailySchedule(int consumerID)
        {
            InitializeComponent();
            ...
            // Load the daily schedule when the form is loaded
            this.Load += new System.EventHandler(this.Form10_DailySchedule_Load);
        }

        private void Form10_DailySchedule_Load(object sender, EventArgs e)
        {
            LoadDailySchedule();
        ...
        private void LoadDailySchedule()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                // Fetch Appliances
                string queryAppliances = "SELECT * FROM Appliances WHERE ConsumerID = @ConsumerID AND Category NOT IN ('Weekly', 'Monthly')";
                using (SqlCommand cmd = new SqlCommand(queryAppliances, conn))
                {
                    ...
                    using (SqlDataAdapter adapter = new SqlDataAdapter(cmd))
                    {
                        ...
                        foreach (DataRow appliance in appliancesTable.Rows)
                        {
                            ...
                            scheduleTable.Rows.Add(applianceName, startTime.ToString(@"hh\:mm"), endTime.ToString(@"hh\:mm"));
                        }
